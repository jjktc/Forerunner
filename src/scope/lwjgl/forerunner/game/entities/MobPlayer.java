...
public class MobPlayer extends EntityMob {
	...
	public void renderHud() {
		...
		Sprites.hud_health_bg.render(8, 8, Sprites.hud_health_bg.width, (int) (Sprites.hud_health_bg.height * lifePercent), hudR, hudG, hudB, 0.5f);
		Sprites.hud_health.render(8, 8, 1, hudR, hudG, hudB, 0.5f);
		Fonts.fontDefault.drawString(lives + "%", 8 + ((Sprites.hud_health.width) - Fonts.fontDefault.getWidthByString(lives + "%", 2)) / 2, 8 + ((Sprites.hud_health.height) - Fonts.fontDefault.getHeightByString(lives + "%", 2)) / 2, 2, hudR, hudG, hudB, 0.75f);
		Sprites.hud_energy.render((Main.width - Sprites.hud_energy.width) / 2, 8, 1, hudR, hudG, hudB, 0.5f);
		Sprites.hud_energy_section.render((Main.width - Sprites.hud_energy.width) / 2 + 1, 8, (int) (energyPercent * (Sprites.hud_energy.width - 2)), 66, hudR, hudG, hudB, 0.75f);
		HudMap.render();
		Sprites.hud_weaponHolder.render((int) (Main.width - Sprites.hud_weaponHolder.width * w1Scale - Sprites.hud_weaponHolder.width * w2Scale), 0, w1Scale, hudwhR, hudwhG, hudwhB, 0.5f);
		Sprites.hud_weaponHolder.render((int) (Main.width - Sprites.hud_weaponHolder.width * w2Scale), 0, w2Scale, hudwhR2, hudwhG2, hudwhB2, 0.5f);
		primary.weaponSprite.render((int) (Main.width - Sprites.hud_weaponHolder.width * w1Scale - Sprites.hud_weaponHolder.width * w2Scale) + (int) (((Sprites.hud_weaponHolder.width - primary.weaponSprite.width) * w1Scale) / 2), (int) (((Sprites.hud_weaponHolder.height - primary.weaponSprite.height) * w1Scale) / 2), w1Scale);
		secondary.weaponSprite.render((int) (Main.width - Sprites.hud_weaponHolder.width * w2Scale) + (int) (((Sprites.hud_weaponHolder.width - secondary.weaponSprite.width) * w2Scale) / 2), (int) (((Sprites.hud_weaponHolder.height - secondary.weaponSprite.height) * w2Scale) / 2), w2Scale);
		Fonts.fontDefault.drawString(selectedWeapon.name, (int) (Main.width - Fonts.fontDefault.getWidthByString(selectedWeapon.name, 1) - 8), 2 + Sprites.hud_weaponHolder.height, 1);
		if(!aiming) Sprites.hud_aimer.render((Main.width - Sprites.hud_aimer.width * 2) / 2, (int) (((Main.height - Sprites.hud_aimer.height * 2) / 2)), 2, 1, 1, 1, 0.9f);
		Fonts.fontDefault.drawString(selectedWeapon.clip.rounds + "/" + selectedWeapon.ammo, Main.width - Fonts.fontDefault.getWidthByString(selectedWeapon.clip.rounds + "/" + selectedWeapon.ammo, 1) - 4, Main.height - Fonts.fontDefault.colH - 4, 1);
		/*if(!Tutorial.finished) {
			Tutorial.render();
		}*/
	}
	...
	public void controlReload(boolean e) {//E
		if(!selectedWeapon.reloading) {
			if(e) {
				if(selectedWeapon.clip.rounds < selectedWeapon.clip.maxRounds) selectedWeapon.reload();
			}
		}
	}
