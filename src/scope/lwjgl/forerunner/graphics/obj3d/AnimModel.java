...
public class AnimModel {
	...
	public void update() {
		if(currentAnim >= 0) {//an animation is running
			if(!animations[currentAnim].ckf.equals(animations[currentAnim].keyFrames[animations[currentAnim].keyFrame])) {//the current keyframe does not equal what the current keyframe should be
				...
				if(animations[currentAnim].eTime >= animations[currentAnim].keyFrames[animations[currentAnim].keyFrame].time) {
					animations[currentAnim].ckf.setTo(animations[currentAnim].keyFrames[animations[currentAnim].keyFrame]);
			...
			} else {//the keyframe is already ready so wait the set time out before switching to the next keyframe or ending
				animations[currentAnim].eTime += Time.anim;
				if(animations[currentAnim].eTime >= animations[currentAnim].keyFrames[animations[currentAnim].keyFrame].time) {
					animations[currentAnim].ckf.setTo(animations[currentAnim].keyFrames[animations[currentAnim].keyFrame]);
					animations[currentAnim].eTime = 0;
	...
	public void debug() {
		...
		for(int i = 0; i < animations[0].keyFrames.length; i++) {
			printLog2 += animations[0].keyFrames[i].x + " " + animations[0].keyFrames[i].y + " " + animations[0].keyFrames[i].z + "\n";
		}
