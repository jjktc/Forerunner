...
public class DLCWeapon extends DLCContent {
	...
	public void init() {
		if (parameters.size() >= 12) {
			...
			WeaponGun gun = new WeaponGun(name, gpos, clip, weaponSprite, Models.models.get(Models.models.size() - 1),
					automatic, 0, 0, rateOfFire, range, defaultAmmo, ammo);// 0 = damage, 0 = accuracy
			Guns.guns.add(gun);
		} else {
