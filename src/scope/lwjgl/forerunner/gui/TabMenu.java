...
public class TabMenu {
	...
	public TabMenu(int tabSX, int tabSY, float tabScale, Tab[] tabs, Runnable rupdate, Runnable rrender,
			boolean center) {
		this.tabSX = tabSX;
		this.tabSY = tabSY;
		...
		for (int i = 0; i < tabs.length; i++) {
			...
			int tx = (int) (nx
					+ (tabs[i].sprite.width - Fonts.fontDefault.getWidthByString(tabs[i].title, tabScale)) / 2);
