...
public class ClickerText {
	...
	public ClickerText(String text, int x, int y, float scale, ColorRGB color, ColorRGB color2, Runnable method,
			boolean blocked) {
		this.text = text;
		this.x = x;
		this.y = y;
	...
	public void render() {
		...
		case 1:
			...
			Sprites.selectorL.render((int) (x - Sprites.selectorL.width * scale / 2) - 2,
					(int) ((y + height / 2) - (Sprites.selectorL.height * scale / 2) / 2), scale / 2, 1, 1, 1, opacity);
			Sprites.selectorR.render((int) (x + width + 2),
					(int) ((y + height / 2) - (Sprites.selectorR.height * scale / 2) / 2), scale / 2, 1, 1, 1, opacity);
			break;
		case 2:
			Fonts.fontDefault.drawString(text, x, y, scale, color2.r, color2.g, color2.b, opacity);
			Sprites.selectorL.render((int) (x - Sprites.selectorL.width * scale / 2) - 2,
			...
			break;
		case 4:
			Fonts.fontDefault.drawString(text, x, y, scale, color.r / 8 * 7, color.g / 8 * 2, color.b / 8 * 2,
					opacity / 8 * 7);
			break;
		}
	}
	...
	public void renderSelector() {
		...
		Sprites.selectorL.render((int) (x - Sprites.selectorL.width * scale / 2) - 2,
				(int) ((y + height / 2) - (Sprites.selectorL.height * scale / 2) / 2), scale / 2, 1, 1, 1, 0.9f);
		Sprites.selectorR.render((int) (x + width + 2),
				(int) ((y + height / 2) - (Sprites.selectorR.height * scale / 2) / 2), scale / 2, 1, 1, 1, 0.9f);
	}
