...
public class StateMainMenu extends State {
	...
	public ClickerText ctBack = new ClickerText("Back", (160 - Fonts.fontDefault.getWidthByString("Back", 2) / 2), 442, 2, new ColorRGB(1, 1, 1, 0.3f), new ColorRGB(0.6f, 0.6f, 1f, 0.3f), new Runnable() {
		public void run() {
			stateMain.enter();
	...
	public void update() {
		...
		if(stateMain.viewable) {
			...
			if(Options.useJoystick) Gamepad.updateLauncherSelection(1, ctBack);
			ctBack.update();
	...
	public void render() {
		...
		Fonts.fontDefault.drawString(Main.version, (Main.width - Sprites.title.width * 2) / 2 + Sprites.title.width * 2 + 4, Sprites.title.height, 2);
		Fonts.fontDefault.drawString("SCOPE Studios™", (Main.width - Fonts.fontDefault.getWidthByString("SCOPE Studios™", 1)) / 2, 8 + Sprites.title.height * 2, 1);
		if(stateMain.viewable) {
			stateMain.render();
		} else if(stateSingleplayer.viewable) {
			...
			if(Options.useJoystick) Gamepad.renderLauncherSelection(1, ctBack);
			ctBack.render();
		} else if(stateSettings.viewable) {
