...
public class StateMultiplayerGameInfo extends State {
	...
	public ScrollBar scrollbar = new ScrollBar(Main.width - Sprites.scrollbar_bar.width, 0,
			Main.width - Sprites.scrollbar_bar.width, 0, Sprites.scrollbar_bar.width, Main.height, 2,
			Sprites.scrollbar_btn, Sprites.scrollbar_btn_hover, Sprites.scrollbar_btn_clicked, Sprites.scrollbar_bar,
			false);
	public Btn help = new Btn(Sprites.btn_help, Sprites.btn_help_hover, Sprites.btn_help_clicked, "",
			Fonts.fontDefault.getWidthByString("Multiplayer", 2), 0, 1, 1, new Runnable() {
				public void run() {
					helpOpen = true;
				}
			}, false);
	public Btn up = new Btn(Sprites.btn_up, Sprites.btn_up_hover, Sprites.btn_up_clicked, "", Main.width / 5 * 4 - 4,
			(Main.height - Sprites.btn_up.height * 3) / 2 - Sprites.btn_up.height * 3 - 4, 3, 1, new Runnable() {
				public void run() {
					if (helpCurrent < MPGameModes.gameModes.length - 1)
						helpCurrent++;
				}
			}, false);
	public Btn down = new Btn(Sprites.btn_down, Sprites.btn_down_hover, Sprites.btn_down_clicked, "",
			Main.width / 5 * 4 - 4, (Main.height - Sprites.btn_up.height * 3) / 2 + 4, 3, 1, new Runnable() {
				public void run() {
					if (helpCurrent > 0)
						helpCurrent--;
				}
			}, false);
	public ClickerText closeHelp = new ClickerText("Close",
			(Main.width - Fonts.fontDefault.getWidthByString("Close", 2)) / 2,
	...
	public void render() {
		...
		if (helpOpen) {
			...
			Sprites.grey_section.render((Main.width - Sprites.grey_section.width) / 2,
					(Main.height - Sprites.grey_section.height) / 2);
			up.render();
			down.render();
			Sprites.mplayer_modes[helpCurrent].render(Main.width / 4 - 90,
					((Main.height - Sprites.grey_section.height) / 2) + 9);
			Fonts.fontDefault.drawLines(gmHelp.get(helpCurrent), (Main.width - Sprites.grey_section.width) / 2 + 12,
					((Main.height - Sprites.grey_section.height) / 2) + 12 + Sprites.mplayer_modes[0].height, 1);
			closeHelp.render();
		}
	}
